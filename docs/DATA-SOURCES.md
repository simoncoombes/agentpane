...
| TurnEnded | transcript | `{"type":"system","subtype":"turn_duration"}` in the MAIN jsonl. Main finished a turn, which settles the run. Deliberately NOT SessionIdle: a background job session ends turn after turn with no human near it, and on one such session the 40 turn ends had a median of 125 s and a p90 of 627 s before the next tool call — read as idleness that is a pane sitting on the idle screen for minutes at a time while the work carries on. Whether the session is WAITING for someone is the registry's question | `[captured 2026-09-15]` |
