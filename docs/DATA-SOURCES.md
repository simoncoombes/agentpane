...
`/Users/simoncoombes/.local/share/claude/versions/2.1.220`. Evidence provenance:

| Tag | Meaning |
|---|---|
| `[captured]` | Real payload/file captured live on this machine (experiment artifacts in `/private/tmp/claude-501/-Users-simoncoombes-Dev/3e30d225-2fb5-4c3b-b03f-e69f50dd19f0/scratchpad/hookexp/`, or parsed from existing hook scripts / on-disk transcripts) |
...
- `~/.claude/settings.json` is a **symlink into `/Users/simoncoombes/Dev/claude-fleet`**
  (installed by claude-fleet's `install.sh`). Any installer edit lands in that repo's working
...
`/Users/simoncoombes/Dev/.claude/settings.json` exists. Note `permissions.defaultMode` is
...
```json
{"session_id":"292ef2e2-...","transcript_path":"/Users/simoncoombes/.claude/projects/<munged-cwd>/292ef2e2-....jsonl","cwd":"<dir>","hook_event_name":"SessionStart","source":"startup"}
```
...
- **Munging** `[captured]`: cwd with `/` (and presumably other non-alphanumerics) → `-`:
  `/Users/simoncoombes/Dev` → `-Users-simoncoombes-Dev`. Reverse mapping is ambiguous — confirm
...
5. **Installer target**: `~/.claude/settings.json` is a symlink into the claude-fleet repo.
