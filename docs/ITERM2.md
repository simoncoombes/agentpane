...
- **Autopane (recommended).** `./install.sh --autopane` adds a `SessionStart`
...
- **Uninstall.** `./install.sh --uninstall --autopane` removes just this
  entry; plain `./install.sh --uninstall` removes it along with the hook
...
`agentpane hook` is how the TUI sees events. `./install.sh` (section 2)
...
- the contention row `⚠ 2 agents editing db/schema.ts`

`p` pauses the replay, `n` steps one event at a time, and `--demo-seek <sec>`
...
✔ config: /Users/simoncoombes/.config/agentpane/config.toml (defaults apply for missing keys)
...
✖ hooks: no 'agentpane hook' entry in /Users/simoncoombes/.claude/settings.json — paste this into its "hooks" section:

[ ...the settings.json snippet, reproduced in full in section 4 above... ]

  competing settings — read-only scan of /Users/simoncoombes/.claude/settings.json:
⚠ /Users/simoncoombes/.claude/settings.json is a symlink -> /Users/simoncoombes/dev/claude-fleet/settings.json
    these settings look managed by another system (a dotfiles repo or fleet-style installer)
    an install.sh edit will land in /Users/simoncoombes/dev/claude-fleet/settings.json; that system's installer may overwrite it,
...
✔ agentpane entries: none in /Users/simoncoombes/.claude/settings.json — the hooks line above prints the snippet to paste
✔ autopane: auto-open on session start: not installed — ./install.sh --autopane

✔ registry: /Users/simoncoombes/.claude/sessions readable, 4 live session(s)
...
✔ transcripts: /Users/simoncoombes/.claude/projects/-Users-simoncoombes-Dev-agentpane exists
✔ state dir: /Users/simoncoombes/.local/state/agentpane writable (current-<session>.json, runs/s-<session>/)
✔ run store: /Users/simoncoombes/.local/state/agentpane/runs writable
...
- `competing settings` - a read-only scan of `~/.claude/settings.json`,
  running the same inspection `./install.sh` performs before it writes: same
...
- `⚠ ... is a symlink ->` - the settings file is a symlink into a
  dotfiles/fleet repo (here `claude-fleet`), so another system manages it. Any
  edit - install.sh's included - lands in the target, where that system's
...
- `✔ agentpane entries:` - doctor's richer view of agentpane's own hook
  ...
  events and suggests `./install.sh`; a command whose binary path no longer
  ...
  reads the last value; install.sh refuses such a file), and `✖` lines for
...
- `✔ autopane:` - whether the auto-open-on-session-start entry (section 2)
  ...
  installed points at `./install.sh --autopane`. Informational either way —
