...
// TestDocsStatusBarRecipeIsRunnable: the documented recipe used
...
//
// Every markdown file is read, not just the README: the recipe has already
// moved between files once, and a test pinned to one path stops testing
// anything the moment it does.
func TestDocsStatusBarRecipeIsRunnable(t *testing.T) {
	root := filepath.Join("..", "..")
	found := false
	err := filepath.WalkDir(root, func(p string, d os.DirEntry, err error) error {
		if err != nil || d.IsDir() || filepath.Ext(p) != ".md" {
			return nil //nolint:nilerr // an unreadable corner is not the subject
		}
		...
		text := string(data)
		if strings.Contains(text, "$CLAUDE_SESSION_ID") {
			t.Errorf("%s still uses $CLAUDE_SESSION_ID — Claude Code sets CLAUDE_CODE_SESSION_ID", p)
		}
		if strings.Contains(text, `--session "$CLAUDE_CODE_SESSION_ID"`) {
			found = true
		}
		return nil
	})
	...
	if !found {
		t.Error("no doc carries the status-bar recipe with a QUOTED $CLAUDE_CODE_SESSION_ID")
	}
}
