...
// guard: it reads install.sh itself and asserts the shared pattern strings.
...
func TestInspectPartial(t *testing.T) {
	...
	if !detailContains(f, "./install.sh") {
		t.Fatalf("partial coverage must suggest ./install.sh:\n%s", dump(fs))
	}
}
...
func TestInspectStalePath(t *testing.T) {
	...
	if !detailContains(f, "./install.sh") {
		t.Fatalf("stale warn must carry the install.sh fix hint:\n%s", dump(fs))
	}
	if findLine(fs, LevelInfo, "present on all 12 events") == nil {
...
func TestInspectAutopane(t *testing.T) {
	...
	if f == nil || !strings.Contains(f.Line, "./install.sh --autopane") {
...
// where install.sh refuses the file — the scan proceeds on the last value
...
func TestInspectDuplicateKeys(t *testing.T) {
  ...
  "hooks": {}
	...
	if !strings.Contains(f.Line, "last value") || !detailContains(f, "install.sh refuses") {
