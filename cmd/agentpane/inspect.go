...
func InspectSettings(raw []byte, settingsPath, resolvedPath string, binaryPath string) []Finding {
	...
	if resolvedPath != "" {
		add(LevelWarn,
			...
			fmt.Sprintf("an installer edit will land in %s; that system's installer may overwrite it,", resolvedPath),
	...
	if err := json.Unmarshal(raw, &top); err != nil {
		add(LevelFail, fmt.Sprintf("%s is not valid JSON (%v) — the installer refuses to touch it; fix the file by hand", settingsPath, err))
		return findings
	}
	data, ok := top.(map[string]any)
	if !ok {
		add(LevelFail, fmt.Sprintf("%s does not contain a JSON object at the top level — the installer refuses to touch it", settingsPath))
		return findings
	}
	...
	if key := firstDuplicateKey(raw); key != "" {
		add(LevelWarn, fmt.Sprintf("duplicate key %q in %s — this scan (like Claude Code) reads the last value only", key, settingsPath),
			"the installer refuses to touch a file with duplicate keys; fix the duplicate by hand")
	}
	hooks := map[string]any{}
	if hooksAny, has := data["hooks"]; has {
		hm, isObj := hooksAny.(map[string]any)
		if !isObj {
			add(LevelFail, fmt.Sprintf("the \"hooks\" key in %s is not an object — the installer refuses to touch it", settingsPath))
		} else {
			hooks = hm
			for _, ev := range hookEvents {
				if v, present := hm[ev]; present {
					if _, isList := v.([]any); !isList {
						add(LevelFail, fmt.Sprintf("hooks.%s in %s is not a list — the installer refuses to touch it", ev, settingsPath))
					}
				}
			}
		}
	...
	default:
		add(LevelWarn, fmt.Sprintf("agentpane entries: partial coverage — missing %s", strings.Join(missing, ", ")),
			"run `agentpane install` to append the missing events (existing hook groups are left untouched)")
	}
	...
	for _, p := range sortedKeys(staleEvents) {
		add(LevelWarn, fmt.Sprintf("stale agentpane path on %s: %s no longer exists", strings.Join(staleEvents[p], ", "), p),
			...
			"run `agentpane install` to rewrite the binary path in place (trailing arguments are preserved)")
	}
	for _, p := range sortedKeys(differsEvents) {
		add(LevelWarn, fmt.Sprintf("agentpane path on %s is %s — not the running binary (%s)", strings.Join(differsEvents[p], ", "), p, binaryPath),
			"hook events go to that other binary; run `agentpane install` to rewrite the path in place if this one should receive them")
	}
	if len(dupes) > 0 {
		add(LevelWarn, fmt.Sprintf("duplicate agentpane entries on %s", strings.Join(dupes, ", ")),
			"each duplicate forwards the same event twice; `agentpane install` prunes exact duplicates on re-run")
	}
	...
	if len(autopaneCmds) > 0 {
		...
		add(LevelInfo, "autopane: auto-open on session start: not installed — `agentpane install --autopane`")
	}
