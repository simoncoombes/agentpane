...
// Link is the OSC 8 sequence with no enable check, for a renderer that has
// already decided links are on and needs the escape at the LAST moment.
//
// The renderer measures and truncates plain text, so a link cannot live inside
// the text it wraps: the escape has no display width, and the layer that
// strips control characters out of platform text would eat the ESC and leave
// `]8;;file://…` on the row as literal garbage. So the URL travels beside the
// text and the sequence is added here, after styling, at print time.
