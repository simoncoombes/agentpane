...
func TestActivityPrefersTheToolOverDuration(t *testing.T) {
	...
	f.apply(0, event.ToolStart, "a1", withTool("Read", "design/handoff/SPEC.md"))
	f.apply(0, event.ToolEnd, "a1", withTool("Read", "design/handoff/SPEC.md"),
		withDetail("144ms"))
	...
	if got != "Read design/handoff/SPEC.md" {
	...
	if got := f.m.Snapshot().Agents[0].ActivityLine; got != "Read design/handoff/SPEC.md" {
