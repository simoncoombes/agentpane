...
func TestDemoRunHistory(t *testing.T) {
	runs := DemoRunHistory()
	if len(runs) != 3 {
		t.Fatalf("got %d runs, want 3", len(runs))
	}
	if runs[0].ID != "3f9c" {
		t.Fatalf("newest run ID = %q, want 3f9c (the idle-screen mock)", runs[0].ID)
	}
	wantAgents := []int{8, 4, 3}
	for i, r := range runs {
		if len(r.Agents) != wantAgents[i] {
			t.Errorf("run %s: %d agents, want %d", r.ID, len(r.Agents), wantAgents[i])
		}
		if !r.End.After(r.Start) {
			t.Errorf("run %s: End not after Start", r.ID)
		}
		sum := 0
		for _, a := range r.Agents {
			if a.Name == "" || a.Status == "" || a.Duration <= 0 {
				t.Errorf("run %s: incomplete agent %+v", r.ID, a)
			}
			sum += a.Tokens
		}
		if sum != r.Tokens {
			t.Errorf("run %s: agent tokens sum %d != run total %d", r.ID, sum, r.Tokens)
		}
		if i > 0 && !runs[i-1].End.After(r.End) {
			t.Errorf("runs not newest-first at index %d", i)
		}
	}
	if runs[0].Tokens != 611000 || runs[0].Stalls != 1 {
		t.Errorf("newest run must match the §3.8 mock: 611k tokens, 1 stalled; got %dk, %d", runs[0].Tokens/1000, runs[0].Stalls)
	}
	// Callers get independent copies.
	other := DemoRunHistory()
	runs[0].ID = "mutated"
	runs[0].Agents[0].Name = "mutated"
	if other[0].ID != "3f9c" || other[0].Agents[0].Name != "migrate-writer" {
		t.Error("DemoRunHistory returns shared backing data")
	}
}
