...
func script() []event.Event {
	...
	// audit-users-schema — RUN: grepping createUser, edited src/db/schema.ts.
	audit.started(17000)
	...
	// write-email-index — RUN: reads the schema, edits the index it is named for,
	// latest edit is 0042.sql +84 -3.
	//
	// It used to edit src/db/schema.ts at t=60s, which is the same file and the
	// same second as audit-users-schema's edit above — the pair existed to drive
	// the §2.7 contention block. That block was removed, so the pair drove a row
	// nothing rendered; the edit moved to the file this agent is actually about.
	index.started(19000)
	index.read(22000, "src/db/schema.ts")
	index.tokens(35000, 14000)
	index.edit(60000, 8000, "Edit", "src/db/indexes.ts", 9, 1)
