...
func TestCastCoverage(t *testing.T) {
	...
	t.Run("audit-users-schema: grepping createUser, edited schema.ts", func(t *testing.T) {
		...
		if !edited {
			t.Error("must edit src/db/schema.ts for the contention row")
		}
	})

	t.Run("write-email-index: 0042.sql +84 -3 and schema.ts contention", func(t *testing.T) {
		...
		// Contention requires both editors to be not-done at the snapshot.
		for _, id := range []string{idAudit, idIndex} {
			for _, ev := range agents[id] {
				if ev.Kind == event.AgentReturned {
					t.Errorf("%s returned; contention needs both live", id)
				}
			}
		}
	})
