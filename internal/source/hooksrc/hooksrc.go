...
func listenUnix(path string) (net.Listener, error) {
	...
	// The socket carries prompts, tool inputs and file paths out of the
	// user's session, and it lives in a directory every local account can
	// write to. Default socket permissions are 0777&~umask — commonly 0755 —
	// which lets any local user connect and feed the pane invented agents, or
	// watch a session they were never shown. Narrow it to the owner.
	//
	// There is a sliver of time between bind and chmod. Closing it properly
	// means binding inside a 0700 directory, which changes a path the
	// forwarder and the installer both spell out; the chmod turns a permanent
	// opening into a race an attacker has to already be waiting for.
	if err := os.Chmod(path, 0o600); err != nil {
		ln.Close() //nolint:errcheck // the listen error is what matters
		return nil, err
	}
