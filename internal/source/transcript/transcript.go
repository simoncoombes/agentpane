...
type fileState struct {
	...
	// model/effort are the last pair reported for this file, so a
	// ModelObserved is emitted on the first line that names them and again
	// only when they change — not once per assistant message.
	model  string
	effort string
}
