...
func quietFrame(rows []phantomRow, scroll, width int) []string {
	lines := renderQuietInspector(rows, scroll, width, inspectorMin, NewPalette(2, true))
...
func TestQuietInspectorPaginatesVisibly(t *testing.T) {
	...
	if len(frame) > inspectorMin {
		t.Fatalf("frame = %d lines, cap is %d", len(frame), inspectorMin)
	}
	joined := strings.Join(frame, "\n")
...
func TestInspectorRoutesQuietSelection(t *testing.T) {
	...
	got := strings.Join(quietLinesOf(renderInspector(w, v, 64, 28, quietNow(), NewPalette(2, true))), "\n")
