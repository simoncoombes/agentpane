...
func (l *EventLog) Ingest(ev event.Event) {
	...
	case event.ToolEnd:
		...
		// The tool's own description when it wrote one — the same text the row
		// shows — else the tool and its budgeted target. The exact command is
		// still one `y` away.
		text := strings.TrimSpace(ev.Says)
		if text == "" {
			text = ev.Tool
			if ev.Target != "" {
				text += " " + shortTarget(ev.Target)
			}
		}
		switch {
		case ev.Err != "":
	...
	case event.FileEdited:
		text := "Edit " + shortTarget(ev.Target)
		if ev.LinesAdd > 0 || ev.LinesDel > 0 {
			text = fmt.Sprintf("%s (+%d −%d)", text, ev.LinesAdd, ev.LinesDel)
		}
		l.add(id, LogLine{Text: text})
	case event.FileRead:
		l.add(id, LogLine{Text: "Read " + shortTarget(ev.Target), Kind: lineDim})
