...
func TestQuietSelectionOnlyWhereDrawn(t *testing.T) {
	...
	for _, geom := range []struct {
		...
		t.Run(geom.name, func(t *testing.T) {
			v := newUIState(testConfig())
			ids := v.selectionIDs(w, false, quietLineDrawn(w, geom.cols))
			selectable := false
			for _, id := range ids {
				if id == selQuiet {
					selectable = true
			...
			// Whatever is selectable must exist in the painted frame.
			...
			if drawn != geom.wantSel {
				t.Errorf("selQuiet drawn = %v but selectable = %v — selection and paint disagree",
					drawn, geom.wantSel)
			}
		})
	}
