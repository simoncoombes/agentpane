...
// bandBlocked reports whether the band holds a permission ask — the one state
// where the reader is the blocker, and the only one the chip inverts for.
...
func bandBody(entries []bandEntry, dig *digest, width int, pal Palette) []bodyLine {
	...
	for _, e := range shown {
		...
		if e.resolving {
			// §3.7.8: an answer was seen, the outcome was not. Denying emits
			// nothing at all, so this clears late and must never read as a verdict.
			// The mark sits on the entry it belongs to; the entry's own action
			// (askEntryAction) is what stops the region still asking for an answer.
			label = append(label, seg{" · " + copyResolving, pal.NeedsYou})
		}
		if e.kind == kindStalled && e.what != "" {
			label = append(label, seg{" · " + e.what, pal.Settled})
		}
		add(composeLR(label, line(seg{formatSince(e.waited), pal.NeedsYou}), width))
