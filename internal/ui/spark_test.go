...
// §13.3 Q5: the withdrawn glyphs must not come back through the activity scale.
func TestGaugeDrawsNoWithdrawnGlyphs(t *testing.T) {
	now := demoNow()
	for _, cells := range []int{gaugeCells, 3} {
		got := activityGaugeN(1, true, cells, now)
		if strings.ContainsAny(got, "▁▂▃▄▅▆") {
			t.Errorf("the scale drew a withdrawn glyph: %q", got)
		}
	}
}
