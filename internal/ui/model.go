...
type Config struct {
	...
	// Static suppresses the opening draw-in (--render-once, screenshots): one
	// frame of a tree caught mid-arrival is a broken-looking frame, and a CI
	// golden must be the finished picture.
	Static bool
	...
	// Trace, when set, receives one line per frame that has a transition in
	// flight (--log). It exists because "it just appears" is not a report
	// anyone can act on, and three fixes went by before the real cause showed
	// itself: this makes the pane say what it thinks it is drawing.
...
// animMsg is the sub-second repaint that carries a transition (anim.go). It
// exists only while one is in flight; the steady state is the 1s tick.
type animMsg struct{}
...
func New(cfg Config) *Model {
	...
	// Opening the pane draws the tree in: a session already in flight plays its
	// rows on, cascading, exactly as new ones do. It is the same code path and
	// the same honesty — nothing is shown that is not there — and it is the one
	// moment where the pane can afford to introduce itself.
	//
	// Static skips it (--render-once): a single frame of a tree caught
	// mid-arrival is not a picture of anything.
	m.v.Anim = !cfg.Static
	m.syncWall()
...
func (m *Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case animMsg:
		// A transition is running: repaint. The view clock (syncWall, below)
		// carries the motion; the content clock is left to the events.
		m.animPending = false
		m.dirty = true
...
// trace reports what the pane believes it is animating, once per frame, to the
// debug log. Silent unless --log is on and something is actually in flight.
func (m *Model) trace() {
	if m.cfg.Trace == nil {
		return
	}
	for _, a := range m.world.Agents {
		if d, ok := entering(a, m.v); ok {
			m.cfg.Trace("anim enter id=%s name=%q status=%s d=%dms rows=%d",
				a.ID, a.Name, a.Status, d.Milliseconds(), m.rows)
		}
	}
}

// animCmd keeps the clock ticking at animFrame while any transition is running,
// and stops entirely the moment the last one lands: an idle pane is back to one
// frame a second.
func (m *Model) animCmd() tea.Cmd {
	if m.animPending || !animActive(m.world, m.v) {
		return nil
	}
	m.animPending = true
...
// frameGap is the repaint floor: 10fps normally (C10), and animFrame while a
// transition is in flight — a 70ms stagger coalesced to 10fps is not motion,
// it is two jumps.
func (m *Model) frameGap() time.Duration {
	if animActive(m.world, m.v) {
		return animFrame
	}
	return minFrameGap
}

// refresh re-renders at most every frameGap, scheduling a trailing frame
...
func (m *Model) refresh() tea.Cmd {
	...
	if !m.lastFrame.IsZero() {
		if gap := wall.Sub(m.lastFrame); gap < gapFloor {
			if m.framePending {
				return nil
			}
			m.framePending = true
			return tea.Tick(gapFloor-gap, func(time.Time) tea.Msg { return frameMsg{} })
		}
	}
...
// syncWall advances the view's animation clock (anim.go). It is REAL time in a
// live pane, deliberately not the content clock: the content clock jumps to
// event timestamps and the sources arrive in bursts, so a transcript poll
// carrying three seconds of events would step straight over every animation
// window before a frame was drawn. Under --demo the two are the same, so a
// replay stays deterministic.
func (m *Model) syncWall() {
	// Always the injected wall clock — which is real time in a live pane AND in
	// the demo. The demo used the content clock so a replay would be
	// deterministic, and the content clock steps a second at a time: the demo,
	// the one build anyone can run without Claude, animated nothing. Snapshot
	// tests do not go through the Model and set UIState.Wall themselves, so
	// determinism was never this function's to protect.
	m.v.Wall = m.now()
}
...
// persistRun appends the finished run to the run store (§2.8). Nothing in the
// pane reads it back — the idle screen summarises the run the machine just
// ended (World.LastRun) — but `agentpane runs` does, and that is where the
// runs before this one now live.
...
func buildDigest(w state.World, v *UIState, now time.Time) *digest {
	...
	// Every agent, not the filtered tree order: an agent that RETURNED while
	// the reader was away is the digest's own second-ranked entry, and the
	// returned-agent filter would drop exactly those.
	for _, a := range v.orderedAll(w) {
		if a.Rev <= v.BlurRev {
			continue
		}
		marks[a.ID] = true
