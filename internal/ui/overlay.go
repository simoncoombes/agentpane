...
func renderOverlay(w state.World, v *UIState, cols, rows int, pal Palette) [][]seg {
	...
	add(key("j/k ↑/↓", "select · g/G first/last"))
	add(key("␣", "pin the selected agent's tool calls open"))
	add(key("⏎", "inspect · on the alert band: jump to the prompt"))
	...
	if v.sessionPicker() {
		add(key("[ ]", "browse runs · 1–9 switch session (idle)"))
	} else {
		add(key("[ ]", "browse runs (idle)"))
	}
	add(key("x", copyReadOnly))
	add(key("ctrl-c", "quit agentpane — agents keep running"))
	if v.DemoAvailable {
		add(key("p/n", "demo: pause/resume · step one event"))
	}
	add(nil)
	src := v.SourceName
	if src == "" {
