...
func TestStateSessionEnded(t *testing.T) {
	...
	w := state.World{
		...
		Connected: true,
		LastRun: &state.Run{
			ID: "3f9c-r1", StartedAt: demoNow().Add(-5 * time.Minute), EndedAt: demoNow().Add(-1 * time.Minute),
			Tokens: 42000,
	...
	plain := stripANSI(frame)
	if !strings.Contains(plain, "session ended · [ ] browse runs") {
		t.Errorf("session-ended footer missing:\n%s", plain)
	}
	if !strings.Contains(plain, "RUN 3f9c-r1") {
...
func TestStateAllClear(t *testing.T) {
	...
	if !strings.Contains(stripANSI(frame), "all clear · 4 merged") {
		t.Errorf("all-clear line missing:\n%s", stripANSI(frame))
	}
}
...
// Idle keys: [ ] wrap and 1–9 session switching (§5.1).
func TestIdleHistoryAndSessions(t *testing.T) {
	m, _ := newTestModel(t)
	m.v.Runs = []RunRow{{ID: "r1"}, {ID: "r2"}, {ID: "r3"}}
	switched := []string{}
	...
	if m.v.HistIdx != 2 {
		t.Errorf("[ did not wrap backwards: %d", m.v.HistIdx)
	}
	m.handleKey(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("]")})
	if m.v.HistIdx != 0 {
		t.Errorf("] did not advance: %d", m.v.HistIdx)
	}
