...
func rowLines(t *testing.T, a state.Agent, width int, wide bool, now time.Time) []string {
	...
	b := agentBlock(w, v, a, true, true, 2, false, width, wide, now, NewPalette(2, false))
...
// complete; at 44 the station label and the meta suffixes are gone, and both the
// slug and the right-hand number are intact and untruncated at either width.
func TestNarrowDegradationLadder(t *testing.T) {
	...
	for _, want := range []string{slugText, "42 passed", "38k"} {
	...
	// 1. the station label is not on the row at ANY width now — the pips carry
	//    it, and the word is a gloss the inspector has room for.
	...
	// 3. a passing check is not on the row at any width: it is not news, and a
	//    ✓ beside a ✔ tool call says nothing a reader can act on.
	if strings.Contains(narrow44, "verified") {
		t.Errorf("a passing check took row width:\n%s", narrow44)
	}
...
// columns keeps the failed check until there is no room for anything, and only
// then is the activity text cut.
func TestAFailedCheckIsTheLastThingToGo(t *testing.T) {
	now := colNow()
	a := degradeAgent(now)
	a.Verify = state.VerifyFailed
	// Long enough to crowd the row, short enough that the marker still fits.
	a.ActivityLine = "Run the auth suite"

	got := strings.Join(rowLines(t, a, 64, true, now), "\n")
	if !strings.Contains(got, "✖ verify failed") {
		t.Errorf("a crowded row dropped the failed check:\n%s", got)
	}
...
// The collapsed tail is what a cramped row gives up — never the slug and never
// the right-hand number (§13.3 Q3, with the gauge that used to narrow first now
// gone from the row entirely).
func TestTailGivesWayBeforeTheSlugIsTouched(t *testing.T) {
	now := colNow()
	a := degradeAgent(now)
	a.Retries = 2
	head := rowLines(t, a, 40, false, now)[0]
	if !strings.Contains(head, "run-auth-tests") {
		t.Errorf("the slug was cut: %q", head)
	}
	if !strings.Contains(head, "k/s") {
		t.Errorf("the number was cut: %q", head)
	}
	if strings.ContainsAny(head, gaugeFilled+gaugeEmpty) {
		t.Errorf("a bar came back onto the row: %q", head)
	}
}
