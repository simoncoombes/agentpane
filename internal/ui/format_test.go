...
// Mouse: one click selects by hit-testing the last frame AND opens the row;
...
func TestMouseSelectAndInspect(t *testing.T) {
	...
	m.cols, m.rows = 64, 24
	m.v.Anim = false // this is about hit-testing, not about the draw-in
	...
	if !m.v.InspectorOpen {
		t.Errorf("one click did not open the inspector")
	}
