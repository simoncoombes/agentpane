...
func TestReplayPhantomNeverStealsARealName(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Both settle, and the returned-agent filter is lifted so this stays
			// a test about the phantom and not about the filter: the real row is
			// still there, the phantom still has none.
			after, w2, vAfter := replayView(t, mach, now.Add(time.Second))
			vAfter.ShowFinished = true
			shown, _ := renderFrame(w2, vAfter, 64, 44, now.Add(time.Second), NewPalette(2, false))
			after = stripANSI(shown)
