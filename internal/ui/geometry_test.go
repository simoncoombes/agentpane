	...
	for name, build := range worlds {
		for _, insp := range []bool{false, true} {
			for rows := 12; rows <= 50; rows++ {
				...
				// The footer is present and unclipped: it either lists the keys
				// or, while something is blocked on the reader, names the one
				// key that gets them to where it can be answered.
				foot := stripANSI(lines[rows-1])
				if strings.Contains(foot, "session pane") {
					continue
				}
				if !strings.Contains(foot, "⏎ open") {
