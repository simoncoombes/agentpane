...
**Revision 3.1.** Absorbs feedback round 2: the prose sweep PART 12 implied but didn't finish
(`x`, `failed`, `killed`, the sparkline-on-an-open-call diagram, `retry 2/3`, 62-vs-64,
`stations[5]`, the dangling "§3.6 scenario" reference, the split keymap table, stale mock
filenames), plus two clarifications now written into the text: where station pips live (§3.3) and
how `--oneline` gets its state (§3.20a). The needs-you role also lost its hue — it is bright +
reverse video + `⚑`, so the pane is one hue with four luminance steps (§3.10.2a).

Reconciled with the live data-source investigation of Claude Code 2.1.220:
...
- **Token reporting granularity** — are per-agent token counts incremental (usable as a 1s-bucket
  sparkline, §3.4) or only emitted at end-of-message? Record which; it decides the default `spark`
  metric.
...
- **`FakeSource`** — deterministic replay of **§2.10**, driven by `agentpane --demo`.
...
done --+30s------------------------> decayed (display collapse only, not a status)
```
...
If two agents whose status is not `done` have both emitted `FileEdited` for the same
...
## 2.10 The demo scenario — referenced as §2.10

`--demo` and every test that says "the scenario" means this cast, which is exactly what the §3.1
diagram and both mocks render:

| Agent | Status | Detail |
|---|---|---|
| `main` | run | `refactor auth`, todo `write the migration` |
| `fix-ts2345-fallout` | **ask** | blocked 52s on `rm -rf node_modules/.cache && pnpm rebuild`, `↺2`, deduped `×3` with 2 folded retry ghosts, failing `TS2345 handlers.ts:88` |
| `typecheck-worksp…` | **stuck** | no call open, silent 2m50s — flatline `·······` |
| `run-auth-tests` | run | **open call** `◐ in Bash` 3m12s, allowlisted, `✓ verified` |
| `audit-users-schema` | run | `Grep "createUser" src/**`, editing `src/db/schema.ts` |
| `write-email-index` | run | `Edit 0042.sql +84 −3`, also editing `src/db/schema.ts` → **contention row** |
| `lint-and-format` | done | returned 46s ago → **decayed to one line** |
| `document-constr…` | queued | blocked on `write-email-index` |
| `reviewer` | **teammate** | `◇`, `limited telemetry`, no pips/sparkline/clock |

Plus: an away-digest on first paint (§3.19), three sessions on the idle screen (§3.8a), and three
runs of history (`[`/`]`).

## 2.9 Derived values per agent

`status`, `stations[4]`, `retries`, `lastEventAt`, `sinceLast`, `totalElapsed`, `tokens`,
`linesAdd/Del`, `currentTool`, `currentTarget`, `activityLine`, `openFile`, `lastError`,
`callHistory` (ring buffer, last 20; render last 4), `sparkBuckets` (60 × 1s buckets holding **both**
a token delta and a call count, so `spark` can switch without losing history),
...
Two widths, toggled with `w`, persisted: **wide 64 cols** (default) and **narrow 44 cols**.
...
│ │ migration                                           6s │
│ ├─╮                                                      │
│ │ ╰⚑ fix-ts2345-fallout  ↺2               ▃▅▂▁···   52s │  selected
...
│ ├─╮                                                      │
│ │ ╰⚑ typecheck-worksp…                    ·······  2m50s │
│ │    ⤷ no call open · nothing since the last edit  ▪▪▫·  │
│ ├─╮                                                      │
│ │ ╰● run-auth-tests                    ◐ in Bash   3m12s │
│ │    ⤷ pnpm test --watch · allowlisted     ✓ verified    │
│ ├─╮                                                      │
│ │ ╰● explore-schema                      ▃▅▆▄▂▃▅     3s │
...
├──────────────────────────────────────────────────────────┤
│ j/k ␣ ⏎ f o y · t tokens              since last event → │  footer
└──────────────────────────────────────────────────────────┘
```
...
**Line 1:** `<dot> <slug>[  ↺n][  <collapsed tail>]` … right-aligned
`<sparkline | ◐ in Tool> <sinceLast | token rate>`
**Line 2** (expanded, **only when the slug is lossy**): the full description, dim (§3.16)
**Line 3** (expanded): `⤷ <activity>[ ✓ verified]` … right-aligned **station** + tokens
**Lines 4–7** (expanded): last `max_calls_shown` tool calls, `  │ <glyph> <text><meta>`, dim,
with `+n more` when truncated (§3.17)

**Where the station indicator lives — definitive:**

| Surface | Shows |
|---|---|
| tree row, wide | four pips + station label, then the agent's tokens |
| tree row, narrow | four pips only (label is the first thing dropped, §3.1) |
| inspector | full station row: pips + station name + `✓ verified (inferred)`, or `no station data — limited telemetry` for a teammate (PART 4) |

Nothing else renders stations. `teammate` rows render no pips anywhere in the tree (§3.15).
...
7 cells from `▁▂▃▄▅▆▇` over the trailing 60s, one cell per ~8.5s bucket, `·` for an empty bucket,
scaled per-agent against its own max.

**Metric — `spark = "tokens" | "calls"`, default `tokens`:**

| Metric | Signal | Good at | Blind to |
|---|---|---|---|
| `tokens` (default) | token delta per bucket | an agent thinking/generating — it ticks when no tools fire | anything during an open call (no generation) |
| `calls` | `ToolStart`/`ToolEnd` count per bucket | an agent doing many short tool calls | an agent generating a long response with no calls |

The two are complementary, which is why `tokens` is the default: generation is the longer and
quieter half of a typical agent's life. **Confirm token granularity in `docs/DATA-SOURCES.md`** — if
deltas only arrive at end-of-message rather than incrementally, each message lands in a single
bucket, which is coarse but still informative; if per-agent token deltas turn out to be unavailable
entirely, **fall back to `calls` automatically and report the fallback in `doctor`.** Never
interpolate or smooth a token count to make the chart look busier (C8).

- **Only shown when no call is open** — an open call shows `◐ in <Tool>` instead (§2.6a). This is
  what keeps a long healthy build from flatlining like a wedged agent, and it applies to **both**
  metrics: neither tokens nor calls accrue while a foreground command runs.
...
A settled agent (`done`) older than 30s collapses to one dim line:
`● lint-and-format  returned · 0:34`. Live work must always dominate. Decayed rows stay selectable
and expand on `⏎` into the inspector.

## 3.6 Ordering

`ask` → `stuck` → `run` → `queued` → `done` → `teammate` → `unknown`, stable by spawn order
within each group. **Only attention reorders rows** — nothing else may shuffle them. Animate nothing;
...
2. Action line: permission → `answer in the left pane — ⏎ jumps focus`; stall → `o opens the log · y yanks the command`.
...
| live / working | **2** (normal green) in monochrome mode, else the auto-picked accent (§3.10.3) | **must differ from primary** — never the same index |
| needs you (ask, stuck) | **no hue of its own** — bright (index 9–15 of the same hue) **+ reverse video + the `⚑` glyph** (§3.10.2a) | the one loud role. **Never yellow, never red, never a second hue.** I run a green scheme and any foreign hue fights it. |
...
- **Selected row: reverse video** (`SGR 7`).
- **Band label: reverse video on the `⚑ NEEDS YOU` / `⚑ WHILE YOU WERE AWAY` chip only**, not the
  whole band. The band is otherwise delimited by a rule of `─` in grey.

That's it. No fills, no tinted rows, no gradient anything.

## 3.10.2a Monochrome mode — the default

**Do not introduce a second hue for attention.** The pane runs on one hue (the user's) and four
steps of luminance, so it survives any scheme, a light scheme, and `NO_COLOR` identically:

| Role | Treatment |
|---|---|
| settled | **8** — grey |
| live | **2** — normal green (or the accent when the fg isn't green) |
| primary: names, activity, body | **default fg (39)** — the user's own colour |
| needs you | **bright** (`SGR 1` + the bright index of the same hue) **+ reverse video on the band chip** **+ `⚑` in place of the status dot** |

Consequences that are requirements, not suggestions:

- **Live and primary must never resolve to the same index.** Check at startup (§3.10.3) and report
  the resolved pair in `doctor`. If they would collide and no distinct index is available, make live
  `SGR 2` (dim) against a normal-weight primary rather than letting them match.
- A needs-you row is identified by **three** simultaneous cues — glyph, weight, and the inverse chip
  in the band — so it survives `NO_COLOR` with no loss.
- **Error text is the only non-hue colour permitted in the pane** (ANSI 1/9), and only ever as text
  in the inspector log — never a status, never a glyph, never a row.
...
- On start, query the terminal's palette via `OSC 4 ; <index> ; ? BEL` for indices 1, 2, 3, 5, 6 and
  the default foreground (`OSC 10 ; ? BEL`).
- The needs-you role has no index to pick: it is bright + reverse + glyph (§3.10.2a). Verify only
  that the bright variant is distinguishable from the default fg; if not, rely on reverse video
  alone and report that in `doctor`.
- Compute perceptual distance between the default fg and each candidate. **Prefer index 2 (staying
  inside the user's own hue) whenever it is distinguishable from the default fg** — a monochrome
  pane is the goal (§3.10.2a). Only if 2 is indistinguishable from the fg should you fall back to
  6, then 4, then dim-vs-normal weight.
...
maps each role to how it resolves in my scheme (default-fg green for names and body, ANSI 2 live,
bright + reverse + `⚑` for needs-you, ANSI 8 settled, ANSI 1 for inspector error text);
`mock-neutral-palette.html` shows the same monochrome logic on a neutral palette. Treat the mocks
...
```
status    ● ⚑ ○ ◇ ◌      (⚑ replaces the dot on a needs-you row — §3.10.2a)
tools     ✔ ✖ ⚠ ⚑ ◐ ↺ ⤷
stations  ▪ ▫ ·
trunk     │ ├ ╰ ╮ ─
...
### 3.20a How `--oneline` gets its state

`--oneline` runs as a separate short-lived process (a status-bar command runs on a timer), so it
must not attach to a Source. Instead:

- The running TUI writes a small JSON state file to `~/.local/state/agentpane/current.json` on every
  state change, debounced to at most 1/s: `{session, updated_at, live, asks, stalls, in_call,
  settled, oldest_ask_slug, oldest_ask_age}`.
- `--oneline` reads that file, renders one row, and exits. If the file is missing or its
  `updated_at` is older than 15s it prints `no agentpane running` and exits 0 — never an error, so a
  status bar never shows a stack trace.
- The file is the only IPC. `--oneline` never opens the socket and never reads the transcript.
...
↺2 (inferred) · suspects a stale tsbuildinfo
...
`--oneline` does **not** run the TUI. See §3.20a for how it gets its state.
...
spark = "tokens"          # tokens | calls — what the sparkline plots (§3.4)
...
accent = "auto"           # auto | 2 | 3 | 4 | 5 | 6  (ANSI index, see §3.10.3)
                          # needs-you has NO colour setting — it is bright + reverse + ⚑ (§3.10.2a)
...
- a **Claude Code** profile that opens the session left and `agentpane` in a **64-column** right pane,
...
- verification: `agentpane --demo` in the right pane reproduces §2.10
...
6. Inspector, follow mode, `o` / `y` / `Y` / toasts.
...
- `agentpane --demo` reproduces **§2.10** exactly, including the open-call row, the contention row, the teammate, and the decayed `lint-and-format`.
...
- **Renders correctly in my green-on-black scheme, in a light scheme, and with `NO_COLOR=1`** —
  three snapshot tests, and the `NO_COLOR` one must still distinguish needs-you from live from
  settled (glyph + weight + reverse). No hardcoded hex anywhere in the render path (grep the source
  for `#` in colour positions as part of CI).
- **No second hue anywhere for status.** Only ANSI 8, 2, default-fg, bright, and reverse appear in
  status positions; ANSI 1/9 appears only as inspector log text (test by scanning emitted SGR).
- The resolved live/primary index pair is distinct, and `doctor` prints it.
...
**Revision 3.1 — colour:** the needs-you role no longer has a hue. It is bright + reverse video +
the `⚑` glyph, so the whole pane is one hue with four luminance steps (§3.10.2a). Live is ANSI 2,
primary is the default foreground, and they must never resolve to the same index.
